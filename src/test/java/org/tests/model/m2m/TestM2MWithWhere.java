...
public class TestM2MWithWhere extends BaseTestCase {
  ...
  public void testQuery() throws Exception {
    ...
    List<MnyNode> result = DB.find(MnyNode.class).where().eq("allRelations", node).orderBy("id").findList();
    assertThat(result).extracting(MnyNode::getId).containsExactly(1, 2, 3, 4, 5);

    result = DB.find(MnyNode.class).where().eq("allReverseRelations", node).orderBy("id").findList();
    assertThat(result).extracting(MnyNode::getId).containsExactly(1, 2, 3, 4, 5);

    result = DB.find(MnyNode.class).where().eq("bit1Relations", node).orderBy("id").findList();
    assertThat(result).isEmpty(); // -> to = 1 column: 2 0 2 0 2

    result = DB.find(MnyNode.class).where().eq("bit2Relations", node).orderBy("id").findList();
    assertThat(result).extracting(MnyNode::getId).containsExactly(1, 3, 5);

    result = DB.find(MnyNode.class).where().eq("bit1ReverseRelations", node).orderBy("id").findList();
    // -> from = 1 column: 2 1 3 1 3
    assertThat(result).extracting(MnyNode::getId).containsExactly(2, 3, 4, 5);

    result = DB.find(MnyNode.class).where().eq("bit2ReverseRelations", node).orderBy("id").findList();
